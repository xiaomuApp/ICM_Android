package com.example.icm_android.db;
...
import com.example.icm_android.model.Activity;
import com.example.icm_android.model.Club;
import com.example.icm_android.model.Task;
...
public class DBHandler extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		 ...
		 * 关系表
		 */
		db.execSQL("create table clubs_users("
				+ "clubId text not null,"
				+ "uid text not null)");
		
		db.execSQL("create table clubs_activities("
				+ "clubId text not null,"
				+ "atyId text not null)");
		
		db.execSQL("create table activities_users ("
				+ "atyId text not null,"
				+ "uid text not null)");
		
		db.execSQL("create table activities_tasks("
				+ "atyId text not null,"
				+ "taskId text not null)");
		
		db.execSQL("create table tasks_users("
				+ "taskId text not null,"
				+ "uid text not null)");
	...
	public Club getClubByClubId(String clubId){
		...
		Cursor cursor = db.query("clubs", null,
				"clubId = ?", new String[]{clubId}, null, null, null);
	...
	public int updateClub(Club club){
		...
		values.put("clubId", club.getClubId());
		values.put("clubName", club.getClubName());
		return db.update("clubs", values, "clubId = ?", new String[]{club.getClubId()});
	}
	...
	public User getUserByUid(String uid){
		...
		Cursor cursor = db.query("users", new String[]{"uid", "userName", "userPassword"}, 
	...
	//更新某个用户的信息，给定某个用户信息，主要是uid
		public int updateUser(User user){
			...
			values.put("userName", user.getUserName());
			values.put("userPassword", user.getUserPassword());
			return db.update("users", values, "uid = ?" , new String[]{user.getUid()});
		}
	...
	public Activity getAtyByAtyId(String atyId){
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cursor = db.query("activities", null,
				"atyId", new String[]{atyId}, null, null, null);
		if(cursor.moveToFirst()){
			System.out.println("找到改活动信息"+atyId);
			Activity aty = new Activity(cursor.getColumnName(0), 
					cursor.getString(1), cursor.getString(2),
	...
	//获取所有的活动信息
	public List<Activity> getAllAty(){
	...
	public int updateAty(Activity aty){
		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put("atyId", aty.getAtyId());
		values.put("atyName", aty.getAtyName());
		values.put("atyContent", aty.getAtyContent());
		values.put("deadline", aty.getDeadline());
		return db.update("activities", values, "atyId = ?", new String[]{aty.getAtyId()});
	}
	...
	public int updateTask(Task task){
		...
		return db.update("tasks", values, "taskId = ?", new String[]{task.getTaskId()});
	}
	 ...
	 */
	//添加一组社团与活动关系
	public void addClub_Activity(String clubId, String atyId){
		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put("clubId", clubId);
	...
	//通过clubId找到所有活动数据
	public List<Activity> getAllAtyByClubId(String clubId){
		List<Activity> atyList = new ArrayList<Activity>();
	...
	//通过uid找到所有活动的信息
	public List<Activity> getAllAtysByUid(String uid){
		List<Activity> atyList = new ArrayList<Activity>();
	...
	//通过taskId找到所有活动的信息
	public List<Activity> getAllAtysByTaskId(String taskId){
		List<Activity> atyList = new ArrayList<Activity>();
